 ...
 *
 * <p>Optional: when {@code passage.bootstrap-token} (env {@code PASSAGE_BOOTSTRAP_TOKEN}) is
 * unset/blank the check is a no-op so existing deployments keep working. Operators are encouraged
 * to set it.
 */
@Component
public class SeedGuard {
  ...
  public SeedGuard(@Value("${passage.bootstrap-token:}") String bootstrapToken) {
