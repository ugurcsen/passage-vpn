...
class ConfigReportServiceTest {
  ...
  void setUp() {
    ...
    environment.setProperty("spring.datasource.url", "jdbc:sqlite:./data/opnl.db");
    reportService =
  ...
  private static com.opnl.vpn.network.Daemon daemon(
      int index, String name, int port, Protocol proto, boolean enabled) {
    return com.opnl.vpn.network.Daemon.builder()
        .id("d" + index)
        .daemonIndex(index)
        .name(name)
        .port(port)
