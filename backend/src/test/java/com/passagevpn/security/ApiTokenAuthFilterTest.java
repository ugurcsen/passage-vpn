...
class ApiTokenAuthFilterTest {
  ...
  void authenticatesViaXApiTokenHeader() throws Exception {
    ...
    when(apiTokenService.authenticate("passage_raw")).thenReturn(Optional.of(token));
    MockHttpServletRequest request = new MockHttpServletRequest();
    request.addHeader("X-API-Token", "passage_raw");
  ...
  void authenticatesViaBearerHeaderWithTokenPrefix() throws Exception {
    ...
    when(apiTokenService.authenticate("passage_raw")).thenReturn(Optional.of(token));
    MockHttpServletRequest request = new MockHttpServletRequest();
    request.addHeader("Authorization", "Bearer passage_raw");
    MockHttpServletResponse response = new MockHttpServletResponse();
  ...
  void leavesExistingJwtAuthenticationUntouched() throws Exception {
    ...
    verify(apiTokenService, never()).authenticate("passage_raw");
  ...
  void unknownTokenLeavesNoAuthentication() throws Exception {
    when(apiTokenService.authenticate("passage_unknown")).thenReturn(Optional.empty());
    MockHttpServletRequest request = new MockHttpServletRequest();
    request.addHeader("X-API-Token", "passage_unknown");
    MockHttpServletResponse response = new MockHttpServletResponse();
