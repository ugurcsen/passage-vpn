...
class ConnectionRegistryTest {
  ...
  void retainOnlyKeepsSessionsPresentInLiveView() {
    ...
    registry.register("bob", "bob", "10.8.0.3", null, "203.0.113.9", "daemon-1");
  ...
  @Test
  void indexesTheDualStackVirtualIpv6() {
    ConnectionRegistry registry = new ConnectionRegistry();
    registry.register("alice", "alice", "10.8.0.2", "fd00:1::2", "203.0.113.5", "daemon-0");

    assertThat(registry.byVirtualIp("fd00:1::2")).isPresent();
    assertThat(registry.byVirtualIp("fd00:1::2").get().username()).isEqualTo("alice");

    registry.unregister("alice");

    assertThat(registry.byVirtualIp("fd00:1::2")).isEmpty();
    assertThat(registry.byVirtualIp("10.8.0.2")).isEmpty();
  }
}
